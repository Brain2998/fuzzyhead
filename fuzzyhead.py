...
cursor.execute("""CREATE TABLE IF NOT EXISTS tasks (id text PRIMARY KEY, name text NOT NULL,
    ...
    divide_number integer, cli_args text, rps text, status text NOT NULL, result text, result_id text);""")
conn.commit()
...
def root():
    if flask.request.method == 'POST':
        ...
        cli_args, rps, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""", (task_id, task_name, started, dict_name,
        flask.request.form['fuzzer'], target_ip, divide_number, cli_args, rps, 'Running'))
        conn.commit()
        fuzzing_dict.save(dict_path)
        return script.start_fuzzing(flask.request.form['fuzzer'], dict_path, divide_number, cli_args, rps, task_id, conn, cursor)
...
def getTaskDetails():
    ...
    if (args['getDbData']=='true'):
        ...
        'cli_args': result[0][7], 'rps': result[0][8], 'status': result[0][9], 'result': result[0][10]})))
